...
const getVideoLingoApiBaseUrl = () => process.env.NEXT_PUBLIC_VIDEOLINGO_API_URL || window.location.origin;

type ExportUploadMessage = {
	type: "videolingo:export-uploading" | "videolingo:export-uploaded" | "videolingo:export-upload-failed";
	taskId: string;
	message?: string;
	asset?: { name?: string };
};

function notifyVideoLingo(message: ExportUploadMessage): void {
...
function ExportPopover({
	...
	const uploadToVideoLingo = async ({ buffer, mimeType, extension }: { buffer: ArrayBuffer; mimeType: string; extension: string }) => {
		const taskId = activeProject?.metadata.id;
		if (!taskId) return;

		const fileName = `${activeProject.metadata.name || "edited-video"}${extension}`;
		const formData = new FormData();
		formData.append("file", new File([buffer], fileName, { type: mimeType }));
		formData.append("extension", extension);
		notifyVideoLingo({ type: "videolingo:export-uploading", taskId });

		try {
			const response = await fetch(
				`${getVideoLingoApiBaseUrl()}/api/editor/tasks/${encodeURIComponent(taskId)}/exports`,
				{ method: "POST", body: formData },
			);
			if (!response.ok) {
				const detail = await response.text();
				throw new Error(detail || "Failed to archive export");
			}
			const result = (await response.json()) as { asset?: { name?: string } };
			notifyVideoLingo({ type: "videolingo:export-uploaded", taskId, asset: result.asset });
		} catch (error) {
			notifyVideoLingo({
				type: "videolingo:export-upload-failed",
				taskId,
				message: error instanceof Error ? error.message : "Failed to archive export",
			});
		}
	};
	...
	const handleExport = async () => {
		...
		if (result.success && result.buffer) {
			...
			void uploadToVideoLingo({ buffer: result.buffer, mimeType, extension });
