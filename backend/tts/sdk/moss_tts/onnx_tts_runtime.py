...
DEFAULT_BROWSER_ONNX_OUTPUT_PATH = Path("infer_onnx_output.wav")
...
def _snapshot_download_repo(
    *,
    ...
    endpoints = []
    for endpoint in (configured_endpoint, "https://hf-mirror.com", "https://huggingface.co"):
        if endpoint and endpoint not in endpoints:
            endpoints.append(endpoint)
    errors = []
    for endpoint in endpoints:
        try:
            logging.info("downloading %s from %s", repo_id, endpoint)
            snapshot_download(
                repo_id=repo_id,
            ...
            logging.warning("failed to download %s from %s: %s", repo_id, endpoint, exc)
    raise RuntimeError(
        f"Unable to download {repo_id}. Tried endpoints: " + " | ".join(errors)
    )
...
def ensure_browser_onnx_model_dir(
    ...
    *,
    ...
    if not _default_model_dir_requested(model_dir) and not download_missing_assets:
...
class OnnxTtsRuntime(OrtCpuRuntime):
    def __init__(
        ...
        *,
        ...
        resolved_model_dir = ensure_browser_onnx_model_dir(model_dir, download_missing_assets=True)
        super().__init__(
        ...
        self.output_dir = Path(output_dir or self.model_dir / "generated_audio").expanduser().resolve()
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        ...
        return waveform.T[np.newaxis, :, :].astype(np.float32, copy=False)
