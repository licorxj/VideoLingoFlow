"""s_kie_image_upscale: KIE AI 图片高清放大节点。

直接调用 backend/kieai_sdk（不经过生图接口），支持 KIE market 风格放大模型
...
产物落盘 <task_dir>/cache/images，文件名格式：
...
SUPPORTED_MODELS = ("recraft/crisp-upscale", "topaz/image-upscale")
DEFAULT_MODEL = "recraft/crisp-upscale"
...
def _resolve_image(value, task_dir: str) -> str:
    """解析图片输入：支持单路径 / 列表 / HTTP URL，返回本地绝对路径或 URL。"""
...
async def _upload_image(client, path: str) -> str:
    """上传本地图片到 KIE，返回公开可访问 URL。"""
...
async def _do_upscale(image_in: str, model: str, upscale_factor: str,
    ...
    async with KieClient() as client:
        ...
        if image_in.startswith("http"):
            image_url = image_in
        else:
            image_url = await _upload_image(client, image_in)
            if not image_url:
                raise RuntimeError("图片上传失败：未能取得可访问的 URL")

        # 仅下发模型实际声明的参数
        params = {}
        if "image" in pnames:
            params["image"] = image_url
        if "image_url" in pnames:
            params["image_url"] = image_url
        if "upscale_factor" in pnames:
            params["upscale_factor"] = upscale_factor
        if not params:
            raise RuntimeError(f"模型 {model} 未声明图片输入参数，无法提交放大任务")
...
class S_KieImageUpscale(BaseStep):
    """KIE AI 图片高清放大：image 输入 -> 放大后的图片。"""

    step_id = "kie_image_upscale"
    step_name = "图片高清放大-kie"
    dependencies = []
    ...
    def run(self, task_dir: str, callback: Optional[Callable] = None,
        ...
        # 1. 输入图片
        image_in = _resolve_image(inputs.get("image", ""), task_dir)
        if not image_in:
            raise ValueError("未获取到待放大图片：请在 image 输入口连接图片。")
        ...
        try:
            poll_timeout = int(config.get("poll_timeout") or 600)
        except (TypeError, ValueError):
            poll_timeout = 600

        cache_dir = os.path.join(task_dir, "cache", "images")
        temp_dir = os.path.join(task_dir, "cache", "_kie_upscale_temp", node_id)
        os.makedirs(cache_dir, exist_ok=True)

        if callback:
            callback(10, f"准备放大（{model}）...")
            callback(25, "提交放大任务，等待接口返回...")

        # 3. 执行
        try:
            paths = _run_async(
                _do_upscale(image_in, model, upscale_factor, poll_timeout, temp_dir)
            )
        except Exception as e:
            raise RuntimeError(f"KIE 图片放大失败: {e}") from e

        if not paths:
            raise RuntimeError(
                "图片放大未返回任何产物：请检查 KIE API Key"
                "（【全局设置 → 密钥管理器】中名称必须为 KIEAI_API_KEY）、图片是否有效。"
            )

        # 4. 产物落盘（文件名带 node_id，避免同类型多实例互相覆盖）
        stem = _sanitize(os.path.splitext(os.path.basename(image_in.split("?")[0]))[0])[:24]
        saved = []
        for i, src in enumerate(paths):
            ...
            saved.append(os.path.join("cache", "images", name))
            ...
            json.dump(
                {
                    "node_id": node_id,
                    "model": model,
                    "upscale_factor": upscale_factor if model == "topaz/image-upscale" else "",
                    "source": image_in,
                    "outputs": saved,
        ...
        if callback:
            callback(100, f"已保存 {len(saved)} 张放大图片到 cache/images")

        return {
            "artifacts": list(saved) + [params_rel],
            "outputs": {"image": saved[0], "images": saved, "params": params_rel},
        }
