...
BUILTIN_NODE_TYPES = [
    {
        ...
        "configFields": [
            ...
            {"key": "text", "label": "文本内容", "type": "textarea", "placeholder": "输入文本内容，作为「文本」端点的输出（不落盘）", "dependsOn": "selectedTypes", "dependsAnyValues": ["text"]},
    ...
    },
    {
        ...
        "icon": "Download",
        "color": "#0891b2",
        "inputs": [{"id": "url", "label": "专辑/播放列表 URL", "type": "url", "required": True}],
        "outputs": [
            {"id": "json", "label": "下载产物清单", "type": "json", "color": "#6366f1"},
            {"id": "folder", "label": "产物目录", "type": "text", "color": "#8b5cf6"},
            {"id": "video", "label": "首个视频", "type": "video"},
        ],
        "defaultConfig": {"download_subs": False, "download_cover": False, "resolution": "best", "cookie_file": "", "playlist_items": "", "max_items": 0, "folder_name": ""},
        "configFields": [
            {"key": "download_subs", "label": "下载字幕", "type": "checkbox"},
            {"key": "download_cover", "label": "下载封面", "type": "checkbox"},
            {"key": "resolution", "label": "下载分辨率", "type": "select", "options": [
            ...
            {"key": "playlist_items", "label": "下载范围", "type": "text", "colSpan": "half",
             "placeholder": "留空=全部，如 1-10 / 1,3,5",
             "description": "yt-dlp --playlist-items 表达式，指定下载专辑中的哪些条目"},
            {"key": "max_items", "label": "最多下载条数", "type": "number", "colSpan": "half",
             "min": 0, "max": 1000, "step": 1, "defaultValue": 0,
             "description": "0 表示不限；按专辑顺序只下载前 N 条"},
            {"key": "folder_name", "label": "产物文件夹名", "type": "text", "colSpan": "full",
    ...
    },
    {
        ...
        "configFields": [
            ...
            {"key": "model", "label": "分离模型", "type": "api-select", "apiEndpoint": "/api/separation-interfaces/config-fields?scope=twostem", "dependsOn": "method", "optionLabel": "label", "optionValue": "value", "placeholder": "留空则使用接口默认模型", "colSpan": "full"},
    ...
    },
    {
        ...
        "color": "#84cc16",
        "inputs": [
            {"id": "file", "label": "单文件", "type": "filepath", "required": False},
            {"id": "files", "label": "文件", "type": "list", "required": False,
             "description": "接入文件列表（如多产物/合并列表节点）时逐条登记"}
        ],
        "outputs": [
            {"id": "path", "label": "素材路径", "type": "filepath"},
            {"id": "paths", "label": "素材路径列表", "type": "list"},
            {"id": "count", "label": "入库条数", "type": "text"}
        ],
        "defaultConfig": {},
        "configFields": []
    },
    {
        ...
        "color": "#84cc16",
        "inputs": [
            {"id": "any", "label": "触发", "type": "any", "required": False}
        ],
        "outputs": [
            {"id": "path", "label": "素材路径", "type": "filepath"},
            {"id": "info", "label": "素材信息", "type": "json"}
        ],
        "defaultConfig": {
            "pick_mode": "auto",
            "selected_path": "",
    ...
    },
    {
        ...
        "defaultConfig": {"video_speed_adjust": False, "speed_min": "", "speed_max": "", "gap_threshold": "", "fast_limit": "", "audio_format": "", "audio_bitrate": ""},
        "configFields": [
            ...
            {"key": "fast_limit", "label": "视频变速最大倍率", "type": "text", "placeholder": "留空读取全局 video.speed.fast_limit，默认 1.5",
    ...
    },
    {
        ...
        "configFields": [
            ...
            {"key": "concat_strategy", "label": "拼接策略", "type": "select", "colSpan": "half", "options": [
                 {"value": "low_memory", "label": "低内存（分段转码+流拷贝，推荐）"},
                 {"value": "single_pass", "label": "单次滤镜（快但高内存）"}
    ...
    },
    {
        ...
        "color": "#8b5cf6",
        "inputs": [
            {"id": "asr", "label": "asr格式json", "type": "json", "required": True,
             "description": "接入 ASR 结构化结果（含 segments 句子级时间戳与 words 词级时间戳），作为断句时间轴来源"},
            {"id": "subtitle", "label": "翻译结果JSON", "type": "json", "required": True},
        ],
        "outputs": [{"id": "subtitle", "label": "对齐结果JSON", "type": "json"}],
        "defaultConfig": {"max_subtitle_length": 30, "batch_align": True, "max_request_chars": "", "request_interval": ""},
        "configFields": [
            {"key": "max_subtitle_length", "label": "译文单行最大字符数", "type": "text", "placeholder": "默认 20 字符"},
            {"key": "batch_align", "label": "批量对齐(多句合并请求)", "type": "checkbox", "description": "把同一轮的超长句断句任务按字符预算合并为批量请求（一次请求处理多句）；批内个别任务校验不通过时自动降级为本地切分。关闭则恢复「每句独立请求」的旧行为"},
            {"key": "max_request_chars", "label": "单次请求字数上限", "type": "text", "placeholder": "留空读取全局 llm.max_request_chars", "description": "批量对齐时单次请求的字符预算，用于控制批次大小；留空则使用全局配置（默认 6000）"},
            {"key": "request_interval", "label": "请求最小间隔(秒)", "type": "text", "placeholder": "留空读取全局 llm.min_request_interval", "description": "两次 LLM 请求之间的最小间隔，给上游限速留余量，防止瞬时打爆 429；填 0 表示不限制。留空则用全局配置（默认 0.5 秒）"},
        ],
    },
    {
        "id": "video_preview",
    ...
    },
    {
        ...
        "color": "#14b8a6",
        "inputs": [
            {"id": "audio1", "label": "音轨1", "type": "audio", "required": False},
            {"id": "audio2", "label": "音轨2", "type": "audio", "required": False},
            {"id": "audio3", "label": "音轨3", "type": "audio", "required": False},
            {"id": "audio4", "label": "音轨4", "type": "audio", "required": False},
            {"id": "audio5", "label": "音轨5", "type": "audio", "required": False},
            {"id": "audio6", "label": "音轨6", "type": "audio", "required": False},
        ],
        "outputs": [],
        "defaultConfig": {"sync_play": True, "solo_mode": False},
        "configFields": [],
    },
    {
        "id": "llm_request",
    ...
    },
    {
        ...
        "color": "#0284c7",
        "inputs": [
            {"id": "url", "label": "下载地址", "type": "url", "required": True},
            {"id": "filename", "label": "文件名称", "type": "text", "required": False},
        ],
        "outputs": [
            {"id": "file", "label": "文件路径", "type": "filepath"},
            {"id": "filename", "label": "文件名", "type": "text"},
        ],
        "defaultConfig": {"timeout": 300, "overwrite": True, "headers": ""},
        "configFields": [
            {"key": "timeout", "label": "下载超时(秒)", "type": "number", "min": 5, "max": 86400, "colSpan": "half", "description": "单次读取的超时时间；大文件的整体下载时长通常不受此限制"},
            {"key": "overwrite", "label": "同名文件覆盖", "type": "checkbox", "colSpan": "half", "description": "关闭后遇到同名文件会自动追加 -1 / -2 序号，不覆盖已有文件"},
            {"key": "headers", "label": "附加请求头", "type": "textarea", "placeholder": "{\n  \"Referer\": \"https://example.com/\",\n  \"User-Agent\": \"Mozilla/5.0 ...\"\n}", "description": "可选。JSON 对象；用于防盗链或需要特定 User-Agent / Referer 的资源"},
        ],
    },
    {
        "id": "image_gen",
    ...
    },
    {
        ...
        "outputs": [
            {"id": "result", "label": "执行完成情况", "type": "any"},
        ],
        "defaultConfig": {},
        "configFields": [],
    },
    ...
    },
    {
        ...
        "configFields": [
            {"key": "inputCount", "label": "输入端口数", "type": "number", "min": 1, "max": 8},
            {"key": "outputCount", "label": "输出端口数", "type": "number", "min": 1, "max": 8},
            {"key": "opencode_exe", "label": "CLI 路径", "type": "text", "colSpan": "half", "placeholder": "留空自动探测", "description": "随所选 CLI 生效"},
            {"key": "agent", "label": "主 Agent", "type": "text", "colSpan": "half", "placeholder": "留空用默认主 Agent", "description": "仅部分 CLI 有效"},
            {"key": "model", "label": "模型", "type": "opencode-models", "colSpan": "full", "placeholder": "provider/model；留空用默认模型", "description": "留空用默认模型；claude / codex 需手填"},
            {"key": "fallback_models", "label": "兜底模型", "type": "text", "colSpan": "half", "placeholder": "多个用逗号分隔", "description": "主模型失败时按序回退"},
            {"key": "dir_name", "label": "工作子目录", "type": "text", "colSpan": "half", "placeholder": "留空在任务目录执行", "description": "相对任务目录，自动创建"},
            {"key": "timeout", "label": "会话超时(秒)", "type": "number", "min": 60, "max": 28800, "colSpan": "half"},
            {"key": "auto_approve", "label": "自动放行工具权限", "type": "checkbox", "colSpan": "half", "description": "非交互下权限会被拒绝，建议开启"},
            {"key": "thinking", "label": "输出思考过程", "type": "checkbox", "colSpan": "half"},
            {"key": "pure", "label": "纯净模式(不加载插件)", "type": "checkbox", "colSpan": "half"},
            {"key": "skills", "label": "可调用 Skill", "type": "multiselect", "colSpan": "half", "placeholder": "自行选择", "options": [], "description": "推荐优先使用，留空由智能体自选"},
            {"key": "mcps", "label": "可调用 MCP", "type": "multiselect", "colSpan": "half", "placeholder": "自行选择", "options": [], "description": "推荐优先使用，留空由智能体自选"},
            {"key": "instruction", "label": "任务指令", "type": "textarea", "placeholder": "你是本项目的工作流节点执行者，执行我要求的任务，并按照需要输出产物。本次执行的任务是："},
        ],
    },
    {
    ...
    },
    {
        ...
        "configFields": [
            ...
            },
            {
                ...
                "options": [
                    {"value": "", "label": "自动（跟随全局显卡加速设置）"},
                    {"value": "libx264", "label": "H.264 (libx264)"},
                    {"value": "libx265", "label": "H.265 (libx265)"},
                    {"value": "h264_nvenc", "label": "H.264 显卡加速 (h264_nvenc)"},
                    {"value": "hevc_nvenc", "label": "H.265 显卡加速 (hevc_nvenc)"},
    ...
    },
    {
        ...
        "inputs": [
            ...
            {"id": "in_3", "label": "输入3", "type": "any"},
            {"id": "in_4", "label": "输入4", "type": "any"},
        ],
        "outputs": [
            {"id": "out_1", "label": "输出1", "type": "any"},
            {"id": "out_2", "label": "输出2", "type": "any"},
            {"id": "out_3", "label": "输出3", "type": "any"},
            {"id": "out_4", "label": "输出4", "type": "any"},
            {"id": "result", "label": "输出汇总", "type": "json"},
            {"id": "artifacts", "label": "产物列表", "type": "json"},
        ],
        "defaultConfig": {
            "targetWorkflowId": "",
            "refMode": "snapshot",
        ...
        "configFields": [
            {"key": "targetWorkflowId", "label": "目标工作流", "type": "api-select",
             "apiEndpoint": "/api/workflows", "optionLabel": "name", "optionValue": "id", "colSpan": "full",
             "description": "选择要执行的工作流；选定后会自动生成快照用于执行"},
            {"key": "refMode", "label": "引用方式", "type": "select", "colSpan": "half", "options": [
                {"value": "snapshot", "label": "快照（推荐）"},
                {"value": "live", "label": "实时读取最新定义"},
            ], "description": "快照保证执行可复现；实时引用每次执行都拉取最新工作流定义"},
            {"key": "outputStrategy", "label": "输出取值", "type": "select", "colSpan": "half", "options": [
                {"value": "auto_last_layer", "label": "拓扑最后一层"},
                {"value": "explicit_node", "label": "指定终结节点"},
                {"value": "merge_all", "label": "合并全部输出"},
            ], "description": "配置了「输出映射」时以映射为准，忽略此处策略"},
            {"key": "explicitEndNodeId", "label": "终结节点", "type": "text", "colSpan": "half",
             "dependsOn": "outputStrategy", "dependsValue": "explicit_node",
             "placeholder": "子工作流中的节点 id", "description": "从下方映射面板的节点列表中复制节点 id"},
            {"key": "inputMappings", "label": "输入输出映射", "type": "wf-io-mapping", "colSpan": "full",
             "description": "把本节点输入端口映射到子工作流内部节点的输入端口；把子工作流内部节点的输出端口映射回本节点输出端口"},
            {"key": "workspaceMode", "label": "执行目录", "type": "select", "colSpan": "half", "options": [
                {"value": "subdir", "label": "独立子目录（推荐）"},
                {"value": "inherit", "label": "共用父任务目录"},
            ], "description": "独立子目录在当前任务目录下新建 subwf_<节点id>/，子工作流在其中执行"},
            {"key": "promoteOutputs", "label": "提升产物到父目录", "type": "checkbox", "colSpan": "half",
             "description": "把终点产物复制到父任务 cache/ 并带节点 id 后缀；关闭则下游节点按 find_artifact 找不到子目录中的产物"},
            {"key": "cleanupAfter", "label": "执行后删除子目录", "type": "checkbox", "colSpan": "half",
