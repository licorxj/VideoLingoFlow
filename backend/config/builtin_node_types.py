...
BUILTIN_NODE_TYPES = [
    ...
    },
    {
        ...
        "configFields": [
            {"key": "inputCount", "label": "输入端口数", "type": "number", "min": 1, "max": 8},
            {"key": "outputCount", "label": "输出端口数", "type": "number", "min": 1, "max": 8},
            {"key": "opencode_exe", "label": "opencode 路径", "type": "text", "placeholder": "留空自动探测", "description": "opencode 可执行文件绝对路径；留空则依次尝试 PATH、环境变量 OPENCODE_EXE 与常见安装位置"},
            {"key": "model", "label": "模型", "type": "text", "placeholder": "provider/model，如 opencode/mimo-v2.5-free；留空用 opencode 默认模型", "description": "对应 opencode run -m，格式为 provider/model。默认 opencode/mimo-v2.5-free（免费模型）；留空则用 opencode 自身默认模型"},
            {"key": "fallback_models", "label": "兜底模型", "type": "textarea", "placeholder": "一行一个 provider/model", "description": "主模型调用失败（未授权 / 无支付方式 / 全程无输出超时）时按顺序回退尝试。一行一个或逗号分隔；留空则不回退"},
            {"key": "agent", "label": "主 Agent", "type": "text", "placeholder": "留空用默认主 Agent", "description": "对应 opencode run --agent，需为 primary agent"},
            {"key": "variant", "label": "模型变体", "type": "text", "placeholder": "如 high / max / minimal；留空不指定", "description": "对应 opencode run --variant（推理强度等）", "colSpan": "half"},
            {"key": "timeout", "label": "会话超时(秒)", "type": "number", "min": 60, "max": 28800, "colSpan": "half", "description": "单次 opencode 会话的最长执行时间"},
            {"key": "dir_name", "label": "工作子目录", "type": "text", "placeholder": "留空则在任务根目录执行", "description": "相对任务目录；填写后在该子目录内执行（自动创建）"},
            {"key": "auto_approve", "label": "自动放行工具权限", "type": "checkbox", "description": "对应 --auto。非交互模式下未预授权的工具权限会被自动拒绝，关闭后智能体可能无法读写文件、执行命令"},
            {"key": "thinking", "label": "输出思考过程", "type": "checkbox", "description": "对应 --thinking，将 reasoning 事件一并计入"},
            {"key": "pure", "label": "纯净模式(不加载插件)", "type": "checkbox", "description": "对应 --pure，运行时不加载外部插件"},
