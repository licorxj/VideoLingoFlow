...
async def list_json_templates():
    """Get all prompt templates from prompt_templates.json."""
    svc = get_prompt_service()
    return {"templates": svc.load_json_templates()}
...
async def update_json_template(prompt_id: str, req: UpdateTemplateRequest):
    """Update a single template's system_prompt and/or user_prompt."""
    svc = get_prompt_service()
    update_data = {}
    if req.system_prompt is not None:
        update_data["system_prompt"] = req.system_prompt
    if req.user_prompt is not None:
        update_data["user_prompt"] = req.user_prompt
    if not update_data:
