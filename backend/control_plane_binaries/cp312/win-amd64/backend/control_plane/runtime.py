...
TASK_TRANSITIONS = {
    ...
    "queued": {"running", "paused", "cancelled", "deleting"},
    "running": {"queued", "paused", "stopping", "succeeded", "failed", "cancelled", "deleting", "interrupted"},
    "paused": {"queued", "running", "cancelled", "deleting"},
    "stopping": {"paused", "cancelled", "failed", "deleting", "interrupted"},
    "succeeded": {"queued", "deleting"},
    "failed": {"queued", "deleting", "running", "succeeded"},
    "cancelled": {"queued", "deleting"},
    "deleting": {"deleted", "failed"},
    "deleted": {"queued"},
    # 进程重启/异常中断的任务可恢复或重排
    "interrupted": {"paused", "queued", "deleting", "running", "succeeded"},
}
...
class ResourceTokens:
    def __init__(self, capacities: dict[str, int] | None = None, redis_client=None, prefix: str = "videolingo:resource"):
        defaults = {"cpu": 1, "gpu": 1, "llm": 4, "tts": 2, "io": 4}
        if capacities is None:
            capacities = {resource: self._capacity_from_env(resource, capacity) for resource, capacity in defaults.items()}
    ...
    @staticmethod
    def _capacity_from_env(resource: str, default: int) -> int:
        """资源令牌容量可用环境变量 RESOURCE_CAPACITY_<RESOURCE> 覆盖（如 RESOURCE_CAPACITY_GPU=3）。

        默认值保证单卡安全；显存充足时可调高以获得真正的多任务并发。
        """
        raw = os.getenv(f"RESOURCE_CAPACITY_{resource.upper()}", "").strip()
        if not raw:
            return default
        try:
    ...
    @staticmethod
    def _acquire_slot_lock(base_path: str, capacity: int, timeout: float | None) -> FileLock | None:
        """抢占一个 GPU 槽位锁。

        容量=1 时沿用唯一锁文件 base_path（跨进程/线程互斥，与旧行为一致）；
        容量>1 时在 base_path.0..base_path.{capacity-1} 中抢占一个空闲槽位，
        使多个任务/进程可按容量并行占用 GPU（RESOURCE_CAPACITY_GPU 调高后真正生效）。
        timeout=None 一直等待；超时返回 None。
        """
        paths = [base_path] if capacity <= 1 else [f"{base_path}.{i}" for i in range(capacity)]
    ...
    def acquire(self, resource: str, timeout: float | None = None) -> Iterator[None]:
        ...
        timeout 为 None 时排队等待直到拿到令牌（批量任务多节点并发时按序执行）；
        传数值（秒）则等待到超时，超时未获得抛 ResourceLimitError。
        """
        ...
        if resource == "gpu":
            ...
            gpu_lock = self._acquire_slot_lock(lock_path, self.capacities.get("gpu", 1), timeout)
            if gpu_lock is None:
                if self.redis is not None:
                    self.redis.decr(token_key)
                else:
                    semaphore.release()
                raise ResourceLimitError("本机 GPU 正在执行其他重任务")
        try:
            yield
        finally:
...
def queue_for(resource: str, capabilities: set[str] | None = None) -> str:
    ...
    # 网络请求型节点资源占位值（"free"）与空值统一归入 io 队列
    if not resource or resource == "free":
        resource = "io"
